...
my @data;
    foreach my $plate_rs (@rs) {
        ...
        my $row_data = $self->get_row_plate_based( \@design_list );
...
sub get_row_plate_based {
    my ($self, $design_list) = @_;
...
FROM summaries where design_id IN ($search_condition) ORDER BY design_gene_id;
SQL_END
