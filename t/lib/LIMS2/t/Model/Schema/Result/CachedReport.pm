package LIMS2::t::Model::Schema::Result::CachedReport;
...
LIMS2/t/Model/Schema/Result/CachedReport.pm - test class for LIMS2::Model::Schema::Result::CachedReport
...
sub all_tests : Test(8) {
    ...
    lives_ok { $resultset->create( { %record, 'expires' => $expiry } ) }
    'Inserting new record';    # only add in 'expires' on insert
    my $stored = $resultset->search( \%record )->single();
    ok( $stored, 'Obtained record from the database' );
    my %inflated = $stored->get_columns();
    cmp_deeply(
        \%inflated,
        { %record, expires => ignore() },
