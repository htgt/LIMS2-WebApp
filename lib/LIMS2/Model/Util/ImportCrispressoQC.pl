...
sub get_miseq_well_experiment {
    my ( $model, $file, $miseq_experiment_hash, $well_hash ) = @_;
...
#Secondary script that only deals with non existent miseq well experiments should be run prior to this one to avoid reloading cache
...
sub extract_data_from_path {
    ...
    #Plate id to well name gives well
    my $well_hash = get_well( $model, $well, $plate_hash );
    return 0 unless $well_hash;

    #Query miseq experiment for name(extracted from the path and converted) and plate id
    my $miseq_experiment_hash = get_miseq_experiment( $model, $exp, $miseq_plate_hash );
    return 0 unless $miseq_experiment_hash;

    #Finally, query miseq well experiment for miseq experiment id and well id to get the miseq well experiment id.
    my $miseq_well_experiment_hash = get_miseq_well_experiment( $model, $file, $miseq_experiment_hash, $well_hash );
    return 0 unless $miseq_well_experiment_hash;
