...
WITH RECURSIVE well_hierarchy(root_well_id, process_id, process_type, input_well_id, output_well_id, output_well_name, output_plate_name, output_plate_type, depth, design_id, short_arm_design_id, cassette, cassette_resistance, cassette_promoter, backbone, recombinase, cell_line, gene_id, gene_symbol, crispr_id ) AS (
    SELECT pr_out.well_id, pr.id, pr.type, pr_in.well_id, pr_out.well_id, output_well.name, output_plate.name, output_plate.type_id, 0, pr.design_id, pr.short_arm_design_id, pr.cassette, pr.cassette_resistance, pr.cassette_promoter, pr.backbone, pr.recombinase, pr.cell_line, pr.gene_id, pr.gene_symbol, pr.crispr_id
...
process_data ( id, type, cassette, cassette_resistance, cassette_promoter, backbone, design_id, short_arm_design_id, cell_line, recombinase, gene_id, gene_symbol, crispr_id ) as (
    SELECT p.id, p.type_id, c.name, c.resistance, c.promoter, b.name, pd.design_id, psd.design_id, cl.name, pr.recombinase_id, gene.design_gene_id, gene.design_gene_symbol, pcr.crispr_id
    FROM processes p
