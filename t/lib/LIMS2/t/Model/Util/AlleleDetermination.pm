...
sub all_tests  : Test(129)
{
    ...
    ok my $test_data = test_data( '10_LIMS2_Model_Util_AlleleDetermination.yaml' ), 'fetching test data yaml file should succeed';
    ...
    ok $ne1a_gc_allele_types->{ '10' } eq 'tm1a/wt+bsd_offtarg', 'well 10 should be allele type < tm1a/wt+bsd_offtarg > for stage SEP_PICK';
    ok $ne1a_gc_allele_types->{ '11' } eq 'wt+neo_offtarg/tm1', 'well 11 should be allele type < wt+neo_offtarg/tm1 > for stage SEP_PICK';

    ok $ne1a_gc_allele_types->{ '12' } eq 'potential wt/wt', 'well 12 should be allele type < potential wt/wt > for stage EP_PICK';
    ok $ne1a_gc_allele_types->{ '13' } eq 'potential tm1a/wt', 'well 13 should be allele type < potential tm1a/wt > for stage EP_PICK';
    ok $ne1a_gc_allele_types->{ '14' } eq 'potential tm1e/wt', 'well 14 should be allele type < potential tm1e/wt > for stage EP_PICK';

    ok $ne1a_gc_allele_types->{ '15' } eq 'potential wt/wt', 'well 15 should be allele type < potential wt/wt > for stage SEP_PICK';
    ok $ne1a_gc_allele_types->{ '16' } eq 'potential tm1a/wt', 'well 16 should be allele type < potential tm1a/wt > for stage SEP_PICK';
    ok $ne1a_gc_allele_types->{ '17' } eq 'potential tm1e/wt', 'well 17 should be allele type < potential tm1e/wt > for stage SEP_PICK';
    ok $ne1a_gc_allele_types->{ '18' } eq 'potential wt/tm1', 'well 18 should be allele type < potential wt/tm1 > for stage SEP_PICK';
    ...
    ok $e_gc_allele_types->{ '23' } eq 'potential tm1c/wt', 'well 23 should be allele type < potential tm1c/wt > for stage SEP_PICK';
    ok $e_gc_allele_types->{ '24' } eq 'potential tm1e/wt', 'well 24 should be allele type < potential tm1e/wt > for stage SEP_PICK';
    ok $e_gc_allele_types->{ '25' } eq 'potential tm1f/wt', 'well 25 should be allele type < potential tm1f/wt > for stage SEP_PICK';
    ok $e_gc_allele_types->{ '26' } eq 'potential wt/tm1', 'well 26 should be allele type < potential wt/tm1 > for stage SEP_PICK';
    ok $e_gc_allele_types->{ '27' } eq 'potential tm1a/tm1', 'well 27 should be allele type < potential tm1a/tm1 > for stage SEP_PICK';
    ok $e_gc_allele_types->{ '28' } eq 'potential tm1c/tm1', 'well 28 should be allele type < potential tm1c/tm1 > for stage SEP_PICK';
