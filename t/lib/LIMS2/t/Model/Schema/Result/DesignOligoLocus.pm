...
sub all_tests  : Tests
{
    ...
    my %design_type_record = (
	'id' => 'alien design type',
    );
    my %user_record = (
	'id' => 990,
	'name' => 'alien_user',
    ...
    lives_ok { $assembly_resultset->search(\%assembly_record)->delete() } 'Deleting the existing test records';
    lives_ok { $chromosome_resultset->search(\%chromosome_record)->delete() } 'Deleting the existing test records';
    lives_ok { $design_oligo_resultset->search(\%design_oligo_record)->delete() } 'Deleting the existing test records';
    lives_ok { $design_oligo_type_resultset->search(\%design_oligo_type_record)->delete() } 'Deleting the existing test records';
    lives_ok { $design_resultset->search(\%design_record)->delete() } 'Deleting the existing test records';
    lives_ok { $species_resultset->search(\%species_record)->delete() } 'Deleting the existing test records';
    lives_ok { $user_resultset->search(\%user_record)->delete() } 'Deleting the existing test records';
    lives_ok { $design_type_resultset->search(\%design_type_record)->delete() } 'Deleting the existing test records';

    note("Generating reference data DesignType record");
    lives_ok { $design_type_resultset->search(\%design_type_record)->delete() } 'Deleting any existing test records';
    lives_ok { $design_type_resultset->create(\%design_type_record) } 'Inserting new record';

    note("Generating reference data User record");
    lives_ok { $user_resultset->search(\%user_record)->delete() } 'Deleting any existing test records';
    ...
    lives_ok { $assembly_resultset->search(\%assembly_record)->delete() } 'Deleting any existing test records';
    lives_ok { $assembly_resultset->create(\%assembly_record) } 'Inserting new record';

    note("Generating reference data Chromosome record");
    lives_ok { $chromosome_resultset->search(\%chromosome_record)->delete() } 'Deleting any existing test records';
    lives_ok { $chromosome_resultset->create(\%chromosome_record) } 'Inserting new record';

    note("Generating reference data Design record");
    lives_ok { $design_resultset->search(\%design_record)->delete() } 'Deleting any existing test records';
    lives_ok { $design_resultset->create(\%design_record) } 'Inserting new record';

    note("Generating reference data DesignOligoType record");
    lives_ok { $design_oligo_type_resultset->search(\%design_oligo_type_record)->delete() } 'Deleting any existing test records';
    lives_ok { $design_oligo_type_resultset->create(\%design_oligo_type_record) } 'Inserting new record';

    note("Generating reference data DesignOligo record");
    lives_ok { $design_oligo_resultset->search(\%design_oligo_record)->delete() } 'Deleting any existing test records';
